...
// 메소드를 사용하는 이유는 코드를 재사용하기 위해 사용

public class MethodEx01 {

	// 메소드 오버로딩 : 한 클래스 안에 여러개 메소드 있는 경우
	
	// 사용자 정의 메소드
	// 사용자 정의 메소드는 프로그래머가 직접 호출해야만 실행됨
	static void check() {	// 정적 메소드
		System.out.println("함수 호출 성공");
		return; 			// void의 경우에는 return 생략 가능함
	}
	
	// 값 전달에 의한 메소드 호출(Call by Value 방식)
	static void check(int a) {	// 매개 변수(parameter): int a=30
	...
	// 주소 값 전달에 의한 메소드 호출 방식(Call by Reference 방식)
